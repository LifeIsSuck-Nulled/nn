...
-- ==========================================
-- AUTO UPGRADE LOGIC (SYNC LOCKED)
-- ==========================================
local ItemStatsDB = {}
pcall(function()
    if ShopConfig then
...
if CustomizeRemote then
    CustomizeRemote.OnClientEvent:Connect(function(pcModel, inventory)
        ...
        task.spawn(function()
            ...
            if hasChanges and #upgradesDone > 0 then
...
local function secureBuy(shopId, itemsToBuy)
    task.spawn(function()
        ...
        IsShopping = false
        if shopId == "PcParts" and AutoUpgrade then scanAndUpgradePCs() end
    end)
end
...
togglePC.MouseButton1Click:Connect(function() MasterPC = not MasterPC; handleToggle(togglePC, "💻 PC AUTO-BUY", MasterPC); if MasterPC then triggerInstantSnipe("PcParts", TargetItemsPC) end end)
toggleGrocery.MouseButton1Click:Connect(function() MasterGrocery = not MasterGrocery; handleToggle(toggleGrocery, "🍎 GROCERY AUTO-BUY", MasterGrocery); if MasterGrocery then triggerInstantSnipe("Grocery", TargetItemsGrocery) end end)
...
task.spawn(function()
    while true do
        ...
        local pcs = getMyPCs()
        local existingCards = {}
        for _, child in ipairs(pcStatusScroll:GetChildren()) do if child:IsA("Frame") then existingCards[child.Name] = child end end
            ...
            if not existingCards[pcName] then
                ...
                tBtn.MouseButton1Click:Connect(function()
                    ...
                    if TargetPCsToUpgrade[pcName] then
                        tBtn.BackgroundColor3 = Color3.fromRGB(40, 160, 70); tBtn.Text = "✅ Upgrade: ON"
                    else
                        tBtn.BackgroundColor3 = Color3.fromRGB(150, 40, 40); tBtn.Text = "❌ Upgrade: OFF"
                    end
                end)
            else
        ...
        for _, oldCard in pairs(existingCards) do oldCard:Destroy() end
    ...
    for _, item in ipairs(guiItemsPC) do
        if currentPCFilter == "All" or item.category == currentPCFilter then
            ...
            btn.MouseButton1Click:Connect(function()
                ...
                else TargetItemsPC[item.name] = true; btn.BackgroundColor3 = Color3.fromRGB(40, 160, 70); btn.Text = " ☑ " .. item.name .. " " .. starDisplay; if MasterPC then triggerInstantSnipe("PcParts", {[item.name] = true}) end end
            end)
        end
    end
...
for _, cat in ipairs(sortedCategoryList) do
    ...
    choiceBtn.Size = UDim2.new(1, 0, 0, 30); choiceBtn.BackgroundColor3 = Color3.fromRGB(25, 25, 30); choiceBtn.TextColor3 = Color3.fromRGB(200, 200, 200); choiceBtn.Text = cat; choiceBtn.Font = Enum.Font.GothamMedium; choiceBtn.TextSize = 12; choiceBtn.ZIndex = 11; choiceBtn.Parent = pcDropdownFrame
...
groceryHeader.Size = UDim2.new(0.9, 0, 0, 30); groceryHeader.Position = UDim2.new(0.05, 0, 0, 10); groceryHeader.BackgroundTransparency = 1; groceryHeader.TextColor3 = Color3.fromRGB(255, 255, 255); groceryHeader.Text = "Grocery & Food Selector"; groceryHeader.Font = Enum.Font.GothamBold; groceryHeader.TextSize = 14; groceryHeader.Parent = groceryTab
...
for _, item in ipairs(guiItemsGrocery) do
    local btn = Instance.new("TextButton")
    btn.Size = UDim2.new(1, 0, 0, 30); btn.BackgroundColor3 = Color3.fromRGB(35, 35, 40); btn.Text = " ☐ " .. item.name; btn.TextXAlignment = Enum.TextXAlignment.Left; btn.TextColor3 = Color3.fromRGB(255, 255, 255); btn.Font = Enum.Font.GothamMedium; btn.TextSize = 12; btn.Parent = grocScroll; Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 4)
    btn.MouseButton1Click:Connect(function()
        ...
        else TargetItemsGrocery[item.name] = true; btn.BackgroundColor3 = Color3.fromRGB(40, 160, 70); btn.Text = " ☑ " .. item.name; if MasterGrocery then triggerInstantSnipe("Grocery", {[item.name] = true}) end end
    end)
end
        ...
        if AutoExtinguish and not IsShopping and not IsBusy then
            pcall(function()
                ...
                for _, obj in ipairs(workspace:GetDescendants()) do
                    ...
                    if obj:IsA("ProximityPrompt") and (name:match("extinguish") or obj.ActionText:lower():match("extinguish") or obj.ObjectText:lower():match("fire")) then fireFound = obj; firePart = obj.Parent; break
                ...
                if fireFound and firePart then
                    ...
                    if backpack then
                        for _, t in ipairs(backpack:GetChildren()) do if t:IsA("Tool") and (t.Name:lower():match("extinguish") or t.Name:lower():match("fire")) then extTool = t break end end
                    end
                ...
                if currentClockTime >= 18 or currentClockTime <= 6 then
                    ...
                    for _, obj in ipairs(workspace:GetDescendants()) do
                        if obj:IsA("ProximityPrompt") then
                            ...
                            if act:match("clean") and (objTxt:match("mess") or objTxt:match("glass")) then
                                messFound = obj; messPos = obj.Parent.Position; messType = objTxt:match("glass") and "glass" or "mess"; break
                            end
                        end
                    end
                    ...
                    if messFound and messPos then
                        ...
                        local backpack = Players.LocalPlayer:FindFirstChild("Backpack")
                        if backpack then
                            for _, t in ipairs(backpack:GetChildren()) do if t:IsA("Tool") and t.Name:lower():match(targetToolName) then toolToEquip = t break end end
                        end
                    ...
                    for _, obj in ipairs(workspace:GetDescendants()) do
                        if obj:IsA("ProximityPrompt") then
                            ...
                            if not a:match("sit") and not n:match("chair") and not o:match("chair") then
                                if n:match("laptop") or n:match("server") or o:match("laptop") or o:match("server") then
                                    if obj.Parent and obj.Parent:IsA("BasePart") then
                                        if (obj.Parent.Position - MyCafePos).Magnitude < 85 then closestPrompt = obj break end
                                    end
                                end
                            end
                        end
                            ...
                            for _, obj in ipairs(workspace:GetDescendants()) do
                                if obj:IsA("Model") or obj:IsA("Folder") or obj:IsA("BasePart") then
                                    ...
                                    if isMatch then
                                        ...
                                        if pos and (pos - MyCafePos).Magnitude <= 85 then
