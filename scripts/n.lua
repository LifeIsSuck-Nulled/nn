...
-- ==========================================
-- UI SETUP (LOADED FIRST PARA HINDI MAG-INVISIBLE)
-- ==========================================
...
statusText.Position = UDim2.new(0, 0, 0, 280)
statusText.BackgroundTransparency = 1
statusText.TextColor3 = Color3.fromRGB(150, 150, 150)
statusText.Text = "Status: 🛡️ Anti-Death Active | 📡 Waiting..."
statusText.Font = Enum.Font.GothamSemibold
statusText.TextSize = 12
statusText.Parent = homeScroll

homeScroll.CanvasSize = UDim2.new(0, 0, 0, 320)

local function handleToggle(btn, nameStr, stateVar)
    if stateVar then btn.BackgroundColor3 = Color3.fromRGB(40, 160, 70); btn.Text = nameStr .. ": ACTIVE"
    else btn.BackgroundColor3 = Color3.fromRGB(150, 40, 40); btn.Text = nameStr .. ": OFF" end
end

togglePC.MouseButton1Click:Connect(function() MasterPC = not MasterPC; handleToggle(togglePC, "💻 PC AUTO-BUY", MasterPC) end)
toggleGrocery.MouseButton1Click:Connect(function() MasterGrocery = not MasterGrocery; handleToggle(toggleGrocery, "🍎 GROCERY AUTO-BUY", MasterGrocery) end)
...
-- 🔥 NEIGHBOR BUG FIX: FLAT DISTANCE (X and Z only)
...
-- ==========================================
-- DATA PROCESSING & PC/GROCERY TABS
-- ==========================================
local guiItemsPC = {}
local guiItemsGrocery = {}
local uniqueCats = { ["CPU"] = true, ["Mousepad"] = true }
local knownMousepads = {["Nocturne"]=true, ["Revv"]=true, ["Shadow"]=true, ["Horizon"]=true, ["Fuji"]=true, ["Petal"]=true, ["Sora"]=true, ["Evergreen"]=true, ["Konoha"]=true, ["Kasumi"]=true, ["Wavy"]=true, ["Hanami"]=true, ["Midnight"]=true, ["Azure"]=true, ["Ripple"]=true, ["Hoshi"]=true, ["Japan"]=true, ["Nimbus"]=true, ["Slate"]=true, ["Collage"]=true}
local knownCPUs = {["Snowdrift"]=true, ["PinkDrift"]=true, ["Dark Nexus"]=true, ["Sakura"]=true, ["Polar X"]=true, ["Voltara"]=true, ["Hexora"]=true, ["Vesta"]=true, ["Trifan-Core"]=true, ["Trifan-Lite"]=true, ["Flat Core"]=true, ["Pulse Core"]=true, ["Classic Core"]=true}
...
if ShopConfig and type(ShopConfig.Items) == "table" then
    for itemName, itemData in pairs(ShopConfig.Items) do
        ...
        if cat == "Grocery" then table.insert(guiItemsGrocery, itemEntry) else uniqueCats[cat] = true; table.insert(guiItemsPC, itemEntry) end
    end
end

local function sortItems(a, b) if a.stars ~= b.stars then return a.stars > b.stars else return a.price > b.price end end
table.sort(guiItemsPC, sortItems); table.sort(guiItemsGrocery, sortItems)

-- PC PARTS TAB
local pcDropdownBtn = Instance.new("TextButton"); pcDropdownBtn.Size = UDim2.new(0.9, 0, 0, 30); pcDropdownBtn.Position = UDim2.new(0.05, 0, 0, 10); pcDropdownBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 35); pcDropdownBtn.TextColor3 = Color3.fromRGB(200, 200, 200); pcDropdownBtn.Text = "Category: ALL ▼"; pcDropdownBtn.Font = Enum.Font.GothamSemibold; pcDropdownBtn.TextSize = 12; pcDropdownBtn.Parent = pcTab; Instance.new("UICorner", pcDropdownBtn).CornerRadius = UDim.new(0, 4)
local pcScroll = Instance.new("ScrollingFrame"); pcScroll.Size = UDim2.new(0.9, 0, 1, -55); pcScroll.Position = UDim2.new(0.05, 0, 0, 45); pcScroll.BackgroundTransparency = 1; pcScroll.ScrollBarThickness = 4; pcScroll.Parent = pcTab; local pcLayout = Instance.new("UIListLayout", pcScroll); pcLayout.Padding = UDim.new(0, 4)
local pcDropdownFrame = Instance.new("ScrollingFrame"); pcDropdownFrame.Size = UDim2.new(0.9, 0, 0, 150); pcDropdownFrame.Position = UDim2.new(0.05, 0, 0, 45); pcDropdownFrame.BackgroundColor3 = Color3.fromRGB(20, 20, 25); pcDropdownFrame.ScrollBarThickness = 4; pcDropdownFrame.ZIndex = 10; pcDropdownFrame.Visible = false; pcDropdownFrame.Parent = pcTab; Instance.new("UICorner", pcDropdownFrame); local pcDropLayout = Instance.new("UIListLayout", pcDropdownFrame)
...
local function refreshPCList()
    ...
    for _, item in ipairs(guiItemsPC) do
        if currentPCFilter == "All" or item.category == currentPCFilter then
            local btn = Instance.new("TextButton")
            btn.Size = UDim2.new(1, 0, 0, 30); btn.BackgroundColor3 = TargetItemsPC[item.name] and Color3.fromRGB(40, 160, 70) or Color3.fromRGB(35, 35, 40)
            local starDisplay = string.rep("⭐", item.stars)
            btn.Text = (TargetItemsPC[item.name] and " ☑ " or " ☐ ") .. item.name .. " " .. starDisplay; btn.TextXAlignment = Enum.TextXAlignment.Left; btn.TextColor3 = Color3.fromRGB(255, 255, 255); btn.Font = Enum.Font.GothamMedium; btn.TextSize = 12; btn.Parent = pcScroll; Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 4)
            btn.MouseButton1Click:Connect(function()
                if TargetItemsPC[item.name] then TargetItemsPC[item.name] = nil; btn.BackgroundColor3 = Color3.fromRGB(35, 35, 40); btn.Text = " ☐ " .. item.name .. " " .. starDisplay
                else TargetItemsPC[item.name] = true; btn.BackgroundColor3 = Color3.fromRGB(40, 160, 70); btn.Text = " ☑ " .. item.name .. " " .. starDisplay; end
            end)
        end
    end
...
for _, cat in ipairs(sortedCategoryList) do
    local choiceBtn = Instance.new("TextButton"); choiceBtn.Size = UDim2.new(1, 0, 0, 30); choiceBtn.BackgroundColor3 = Color3.fromRGB(25, 25, 30); choiceBtn.TextColor3 = Color3.fromRGB(200, 200, 200); choiceBtn.Text = cat; choiceBtn.Font = Enum.Font.GothamMedium; choiceBtn.TextSize = 12; choiceBtn.ZIndex = 11; choiceBtn.Parent = pcDropdownFrame
    choiceBtn.MouseButton1Click:Connect(function() currentPCFilter = cat; pcDropdownBtn.Text = "Category: " .. string.upper(cat) .. " ▼"; pcDropdownFrame.Visible = false; refreshPCList() end)
end
task.spawn(function() task.wait(0.2); pcDropdownFrame.CanvasSize = UDim2.new(0, 0, 0, pcDropLayout.AbsoluteContentSize.Y) end)
pcDropdownBtn.MouseButton1Click:Connect(function() pcDropdownFrame.Visible = not pcDropdownFrame.Visible end); refreshPCList()

-- GROCERY TAB
local groceryHeader = Instance.new("TextLabel"); groceryHeader.Size = UDim2.new(0.9, 0, 0, 30); groceryHeader.Position = UDim2.new(0.05, 0, 0, 10); groceryHeader.BackgroundTransparency = 1; groceryHeader.TextColor3 = Color3.fromRGB(255, 255, 255); groceryHeader.Text = "Grocery & Food Selector"; groceryHeader.Font = Enum.Font.GothamBold; groceryHeader.TextSize = 14; groceryHeader.Parent = groceryTab
local grocScroll = Instance.new("ScrollingFrame"); grocScroll.Size = UDim2.new(0.9, 0, 1, -55); grocScroll.Position = UDim2.new(0.05, 0, 0, 45); grocScroll.BackgroundTransparency = 1; grocScroll.ScrollBarThickness = 4; grocScroll.Parent = groceryTab; local grocLayout = Instance.new("UIListLayout", grocScroll); grocLayout.Padding = UDim.new(0, 4)
for _, item in ipairs(guiItemsGrocery) do
    local btn = Instance.new("TextButton"); btn.Size = UDim2.new(1, 0, 0, 30); btn.BackgroundColor3 = Color3.fromRGB(35, 35, 40); btn.Text = " ☐ " .. item.name; btn.TextXAlignment = Enum.TextXAlignment.Left; btn.TextColor3 = Color3.fromRGB(255, 255, 255); btn.Font = Enum.Font.GothamMedium; btn.TextSize = 12; btn.Parent = grocScroll; Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 4)
    btn.MouseButton1Click:Connect(function()
        if TargetItemsGrocery[item.name] then TargetItemsGrocery[item.name] = nil; btn.BackgroundColor3 = Color3.fromRGB(35, 35, 40); btn.Text = " ☐ " .. item.name
        else TargetItemsGrocery[item.name] = true; btn.BackgroundColor3 = Color3.fromRGB(40, 160, 70); btn.Text = " ☑ " .. item.name; end
    end)
end
task.spawn(function() task.wait(0.2); grocScroll.CanvasSize = UDim2.new(0, 0, 0, grocLayout.AbsoluteContentSize.Y + 10) end)

-- ==========================================
-- SECURE SNIPE FUNCTION
-- ==========================================
local function secureBuy(shopId, itemsToBuy)
    task.spawn(function()
        ...
        local returnCF = nil
        if hrp then returnCF = hrp.CFrame; hrp.CFrame = (shopId == "Grocery") and ShopCFrame_Grocery or ShopCFrame_PC; task.wait(1.5) end
        
        for _, item in ipairs(itemsToBuy) do
            local attempts = 0
            while attempts < 15 do
                attempts = attempts + 1
                local stockData = nil; pcall(function() stockData = StockServiceModule:GetAll(shopId) end)
        ...
        task.wait(0.5); if hrp and returnCF then hrp.CFrame = returnCF; task.wait(0.2) end; IsShopping = false
    end)
end
...
stockSync.OnClientEvent:Connect(function(shopId, stockTable)
    local embedsArray, pcParts, mentionEveryone, pcItemsToBuy, groceryItemsToBuy = {}, {}, false, {}, {}
        ...
        for itemName, quantity in pairs(stockTable) do
            if type(quantity) == "number" and quantity > 0 then
                ...
                if category == "Grocery" then
                    ...
                    if TargetItemsPC[itemName] then isTargeted = true; if MasterPC then mentionEveryone = true; table.insert(pcItemsToBuy, {name = itemName, qty = quantity}) end end
                    table.insert(pcParts, { name = tostring(itemName), quantity = quantity, price = tonumber(itemData.Price) or 0, perHour = itemData.PerHour or 0, stars = tonumber(itemData.Stars) or 0, isTarget = isTargeted })
                end
            end
        end
    end
    ...
    table.sort(pcParts, function(a, b) if a.stars ~= b.stars then return a.stars > b.stars else return a.price > b.price end end)
    local currentFields, fieldCount = {}, 0
    local function packEmbed()
        if #currentFields > 0 then table.insert(embedsArray, { title = "📦 " .. tostring(shopId) .. " - PC PARTS", color = 65280, fields = currentFields }); currentFields = {}; fieldCount = 0 end
    end
    for _, item in ipairs(pcParts) do
        ...
        local starDisplay = item.stars == 0 and "N/A" or string.rep("⭐", item.stars)
        local statsDesc = string.format("📦 **Stock:** %d\n💵 **Price:** %s\n⚡ **Per Hour:** %s\n%s\n\u{200B}", item.quantity, tostring(item.price), tostring(item.perHour), starDisplay)
        table.insert(currentFields, { name = displayName, value = statsDesc, inline = false }); fieldCount = fieldCount + 1
    ...
    table.insert(embedsArray, { title = "⏱️ Info", color = 3447003, description = string.format("🔴 **Next Restock:** <t:%d:t> (<t:%d:R>)\n⚙️ **PC:** %s | **Grocery:** %s", nextRestockUnix, nextRestockUnix, (MasterPC and "🟢 ON" or "🔴 OFF"), (MasterGrocery and "🟢 ON" or "🔴 OFF")), footer = { text = "LABA BABY HUB" }, timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ") })
...
-- ==========================================
-- AUTO CLEAN LOOP (NEIGHBOR BUG FIXED + SMART TOOL)
-- ==========================================
task.spawn(function()
    while true do
        task.wait(1.5)
            ...
            pcall(function()
                ...
                if currentClockTime >= 18 or currentClockTime <= 6 then
                    ...
                    for _, obj in ipairs(workspace:GetDescendants()) do
                        if obj:IsA("ProximityPrompt") then
                            ...
                            if act:match("clean") and (objTxt:match("mess") or objTxt:match("glass") or objTxt:match("spill") or objTxt:match("trash")) then
                                -- 🔥 Tinitingnan kung nasa loob ng sarili mong lote
                                if obj.Parent and isInsideMyBase(obj.Parent.Position) then
                                    messFound = obj
                    ...
                    if messFound and messPos then
                        ...
                        -- 🔥 SMART TOOL: Hinding hindi bubunutin ang fire extinguisher
                    ...
                    if closestPrompt and not hasCustomer then
                        ...
                        for _, obj in ipairs(workspace:GetDescendants()) do
                            if obj:IsA("Model") and not Players:GetPlayerFromCharacter(obj) then
                                ...
                                if root and hum and not hum.Sit and (root.Position - laptopPos).Magnitude < 25 then hasCustomer = true break end
                            end
                        end
                    end
