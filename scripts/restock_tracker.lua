...
stockSync.OnClientEvent:Connect(function(shopId, stockTable)
    ...
    -- Extract and separate items into PC Parts and Grocery
    ...
    -- SORTING LOGIC FOR PC PARTS (Lowest Stars first, then Lowest PerHour)
    ...
    -- SORTING LOGIC FOR GROCERY (Sort by price)
    table.sort(groceryFood, function(a, b)
        return a.price < b.price
    end)
    
    -- Helper function to process lists into embeds
    local function buildEmbeds(itemList, isGrocery)
        ...
        for _, item in ipairs(itemList) do
            if #embedsArray >= 8 then break end -- Keep room for the schedule embed at the bottom
    ...
    local nextRestockUnix = currentUnix + 3600 -- Fallback to 1 hour if it fails to calculate
    
    -- Get the exact restock timer from the game's StockService
    ...
    -- Append the schedule to the very bottom
    table.insert(embedsArray, {
